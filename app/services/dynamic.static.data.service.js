...
function DynamicStaticDataService(ConnectService, ResponseFormatter, $q, Data) {
    ...
    function refresh(tablestosync) {
        ...
        tables_to_sync.map( function( table ) {
            ...
            // sync_check_request.data.push({table: table})
            sync_check_request.data.push({table: table, active: 1 })
            // sync_check_request.data.push({table: table, active: 1, sum: mysum})
            // console.log(mysum + ' ' + table);
        })

        //WORKS!!!!!!
        // if (Data.demo) syncCheckCallback(Demo.res.sync_check)
        if (Data.reload && !Data.demo) ConnectService.post(sync_check_request, fail, syncCheckCallback)
